...
src/
├── validators/
│   └── bullet-generation/                 # Should-I-Apply bullet validators
...
│       ├── shared-validators.js           # Task 4: Shared modules (3 validators)
...
**Note:** `src/validators/` structure allows future `resume-analysis/` sibling folder if Resume Analyzer validators are extracted.
...
### Task 4: `shared-validators.js` (3 validators)
...
import { parseOriginalHistory } from '../validators/bullet-generation/history-parser';
import { findBestMatch } from '../validators/bullet-generation/matching-helper';
...
| 4 | Create `shared-validators.js` | Small | Wrapper validators for shared rules |
...
```javascript
// tests/validators/bullet-generation/core-validators.test.js
...
### Integration Tests
```javascript
// tests/validators/bullet-generation/integration.test.js
...
```bash
wc -l src/validators/bullet-generation/*.js     # Verify each module < 500 lines
...
|------|--------|-------|--------|
| `src/validators/bullet-generation/index.js` | Create | ~20 | Export all modules |
| `src/validators/bullet-generation/core-validators.js` | Create | ~400 | 5 core validators |
| `src/validators/bullet-generation/guardrail-validators.js` | Create | ~500 | 8 guardrail validators |
| `src/validators/bullet-generation/content-validators.js` | Create | ~350 | 7 content validators |
| `src/validators/bullet-generation/shared-validators.js` | Create | ~200 | 3 shared validators |
| `src/validators/bullet-generation/secondary-validators.js` | Create | ~100 | 2 secondary validators |
| `src/validators/bullet-generation/history-parser.js` | Create | ~300 | History parsing with fallback |
| `src/validators/bullet-generation/matching-helper.js` | Create | ~150 | Matching and distance helpers |
...
| `tests/validators/bullet-generation/*.test.js` | Create | ~500 | Unit and integration tests |
