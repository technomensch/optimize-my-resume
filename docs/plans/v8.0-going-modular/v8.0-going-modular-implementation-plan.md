...
### Rule 1: Exact Text Match & Extraction Philosophy
When extracting content from one file to another:
- **DEFINITION: "Extract" means "Identify and Copy"** - do NOT remove, cut, or "pull out" the text from the source file. The source `PROJECT-INSTRUCTIONS.md` must remain intact until the very final cleanup phase (v8.5).
...
1. **Extract and Consolidate** GUI/Artifact instructions → new `gui-artifact-config.md` (~300-400 lines)
