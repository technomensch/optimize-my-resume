...
**Version Strategy:** v8.0 (directory restructure, cascade foundation) → v8.1-8.5 (modular features)  
**Created:** 2026-01-12  
...
- If modules extracted FIRST, then directories restructured → would need to update all module paths AGAIN
...
## ⚠️ CRITICAL: Validation Rules for AI Implementation

> [!CAUTION]
> **These rules MUST be followed during implementation to prevent errors.**

### Rule 1: "Shadow Modularization" Extraction Philosophy
This strategy maintains a "Gold Master" while creating an "Optimized Entry Point":
- **PROJECT-INSTRUCTIONS.md (Gold Master):** Identify and Copy only. Content remains intact. Add a reference block indicating the module as a source of truth for modular systems.
- **Project-GUI-Instructions.md (Optimized Entry Point):** Extract and Remove. Duplicate content is removed from this file and replaced with a modular reference block. This demonstrates immediate token savings.
- **COPY the exact text:** No paraphrasing or "improvements" during module creation.
- **Verification:** Show user BOTH the intact "Gold Master" reference and the "Optimized" GUI removal for each module.
...
- If the plan says "extract `<section_name>`" but the exact boundaries are unclear → **ASK**
- If a file reference seems wrong → **ASK**
- If you're unsure about any change → **ASK**
...
- **PROJECT-INSTRUCTIONS.md:** Must state it is the "Gold Master" and require that modules are updated FIRST before syncing the baseline text.
- **Project-GUI-Instructions.md:** Must state it is an "Optimized Entry Point" and redirect logic updates to modules/Gold Master.
- **AI Behavior:** The AI must read and obey these headers during all subsequent development tasks to prevent architectural drift.
...
|---------|--------|-------|-------------|------------|
| **v8.0.0** | `v8.0-directory-functional-restructure` | #27 | Directory restructure → optimization-tools/ | None (foundation) |
...
│   ├── entry-router.md       │   ├── entry-router.md
│   ├── jd-parsing-17-point.md│   ├── jd-parsing-17-point.md
│   └── job-history-v2...md  │   └── job-history-v2-creation.md
├── phase-2/              →   ├── bullet-optimizer/
│   └── evidence-matching.md  │   └── evidence-matching.md
├── phase-3/              →   ├── job-fit-analyzer/
│   ├── incremental-updates.md│   ├── incremental-updates.md
│   ├── re-comparison.md      │   ├── re-comparison.md
│   └── workflow-router.md    │   └── workflow-router.md
└── phase-4/              →   └── narrative-generator/
    └── summary-generation.md     └── summary-generation.md
```
...
1. **Gold Master:** Add reference block to `PROJECT-INSTRUCTIONS.md` (keep all content).
2. **Optimized GUI:** Remove the 8 blocks from `Project-GUI-Instructions.md` and replace with a single reference block to the new module.
...
- ✅ Core file: 85K → 47K tokens (45% reduction)
- ✅ Functional directory names (clear purpose)
- ✅ 6 clean, independent releases
- ✅ All tools work independently
- ✅ 30-45% token savings per session
- ✅ `Project-GUI-Instructions.md` deleted (no more duplicate maintenance)
