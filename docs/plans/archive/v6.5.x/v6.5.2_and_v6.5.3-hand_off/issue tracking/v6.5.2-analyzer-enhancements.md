...
3. Error handling:
   - Free users selecting Opus → API returns error
...
<div className="mb-6">
  ...
  <p className="text-slate-400 text-sm mt-2">
    💡 Sonnet recommended for balanced speed and quality. Opus requires Pro plan.
  </p>
</div>
...
1. ✅ Added `selectedModel` and `modelError` state variables
2. ✅ Created `models` array with 3 options (Haiku/Sonnet/Opus)
3. ✅ Modified `analyzeResume()` to validate model selection
4. ✅ Updated API call to use `selectedModel` instead of hardcoded 'sonnet'
5. ✅ Added Opus Pro plan error detection and auto-fallback to Sonnet
6. ✅ Created model selector dropdown UI with emoji indicators
7. ✅ Added model error display component (yellow warning box)
8. ✅ Modified button to disable when no model selected
9. ✅ Added tooltip and warning text for disabled button
10. ✅ Clear errors when model changes

**Component Structure:**
```
Phase1ResumeAnalyzer
├── State
│   ├── selectedModel: '' (empty until user selects)
│   ├── modelError: '' (for Pro plan warnings)
│   └── ... (existing state)
├── Models Array
│   └── 3 model definitions (Haiku/Sonnet/Opus)
├── Functions
│   ├── analyzeResume() - Modified validation
│   └── ... (existing functions)
└── UI Components
    ├── Model Selector Dropdown (new)
    ├── Model Error Display (new)
    ├── Analyze Button (modified - disabled state)
    └── ... (existing components)
```
...
## Issue #14: Add Token Usage Guidance to Model Selection

**Status:** ✅ IMPLEMENTED (with placement correction needed)  
**Type:** Enhancement + UI  
**Date:** January 8, 2026  
...
### ⚠️ Placement Correction Required:
...
- Token costs vary by model (Haiku cheapest → Opus most expensive)
...
2. **Collapsible Token Info Section** (Option B)
   ...
   - Guidance on resume length → model selection
...
**After:**
```
Select Model *                                    [ℹ️ Token usage info]
┌────────────────────────────────────────────────────────────────┐
│ 💡 Token Usage Guide                                           │
│                                                                 │
│ ⚡ Haiku: Uses fewest tokens (~3K per analysis)               │
│    Best for: Short resumes (1-3 positions), quick analysis     │
│                                                                 │
│ 🎯 Sonnet: Moderate tokens (~5K per analysis)                 │
│    Best for: Most resumes (3-6 positions), balanced quality    │
│                                                                 │
│ ⭐ Opus: Most tokens (~8K per analysis)                       │
│    Best for: Complex resumes (6+ positions), maximum quality   │
│                                                                 │
│ ─────────────────────────────────────────────────────────────  │
│ 💾 Free tier: 500K tokens/day shared across all Claude        │
│ 💡 Multi-phase tip: Start with Haiku/Sonnet to conserve       │
└────────────────────────────────────────────────────────────────┘
...
💡 Not sure? Sonnet works great for most resumes. Click "Token usage info" for details.
```
...
3. **Clicks info button** → Collapsible section expands
4. **Reads guidance:**
...
## Issue #11: Enhanced JSON Parsing Error Handling with Resume Length Guidance
...
• Strategic expansion: Max 2 positions with 4-5 bullets

**Your Options:**

1️⃣ **Shorten your resume:**
   • Remove older positions (4+ jobs back)
   • Reduce bullets in older positions to 1-2

2️⃣ **Analyze in 2 parts:**
   • Part 1: Most recent 3-4 positions only
   • Part 2: Remaining older positions
   • Combine job histories afterward
...
- [ ] Options (1️⃣ 2️⃣) display correctly
- [ ] Guidelines reference actual instruction limits (350-500 words, 3 bullets baseline)

---

## Issue #12: Action Verb Distribution Visualization with Color-Coded Balance Indicators
...
**Current:**
```
Built (Blue)     ██████████████ 35% (7 bullets)
Lead (Orange)    ████ 10% (2 bullets)
Managed (Purple) ████████ 20% (4 bullets)
```

**Enhanced:**
```
Built (Blue)     ⚠️  ██████████████ 35% (7 bullets)   🔴 Too many - diversify
Lead (Orange)    ⚠️  ████ 10% (2 bullets)             🟡 Consider adding more
Managed (Purple) ✓  ████████ 20% (4 bullets)         🟢 Well balanced
Improved (Green) ❌  ██ 5% (1 bullet)                 ⚫ Critical gap
Collaborate (Pink) ✓  ██████ 15% (3 bullets)          🟢 Well balanced
```
...
## 🗂️ Completed Issues Log

### ✅ Issue #13: Model Selection (Haiku/Sonnet/Opus)
...
## Issue #23: [BUG] Error Handling - API Token Session Limit Errors
...
### Implementation Details - Issue #23:
...
2. **User-Friendly Error Message**
   - 🚦 Emoji header for visual recognition
   - Explains what happened (500K tokens/5 hours)
   - Shows when limit resets (countdown + specific time)
   - Provides 3 actionable options:
     1️⃣ Wait for automatic reset
     2️⃣ Upgrade to Pro (5x tokens)
     3️⃣ Strategic token usage tips
...
• Phase 1 analysis uses ~3K-8K tokens...

When will it reset?
• Your limit resets in 2 hours and 15 minutes
• Reset time: 3:45 PM

What you can do:
1️⃣ Wait for reset - Your tokens will automatically refresh...
2️⃣ Upgrade to Pro - Get 5x more tokens...
3️⃣ Use tokens strategically:
   • Haiku uses fewer tokens...
```


## Issue #24: [ENHANCEMENT] Move Repair Recommendations from Executive Summary to Per-Bullet Context

**Status:** ✅ IMPLEMENTED in v6.5.3 (Moved to v6.5.3 documentation)  
**Type:** Enhancement + UI  
**Date Started:** January 8, 2026  
**Date Completed:** January 8, 2026  
**Related Files:** 
- `/mnt/user-data/outputs/Phase1ResumeAnalyzer.jsx` (Artifact - v6.5.3)
- `/mnt/user-data/outputs/v6.5.3-analyzer-enhancements.md` ← **FULL DOCUMENTATION HERE**
...
## 📝 Important Note: UI-Only Changes

**For issues that are UI-only and implemented entirely in the artifact:**

These issues can be marked as ✅ COMPLETED in Git/GitHub once the artifact file is delivered to the user, since:
...
- Issue #13: Model Selection (UI + state management)
- Issue #14: Token Usage Guidance (UI + collapsible info)
- Issue #23: Rate Limit Error Handling (UI + error formatting)
- Issue #24: Move Repair Recommendations (UI + layout restructuring)

**Issues requiring documentation updates:**
- Issue #11: JSON Parsing Error (needs instruction updates for resume length guidelines)
- Issue #12: Verb Distribution Visualization (needs instruction updates for balance thresholds)


---

## 🐛 Bug Fix: Duplicate Function Declaration
...
- ❌ Multi-position (3+): Still truncating at ~17.6K chars (Issue #7 - see handoff doc)

### Known Issues:
- **Issue #25:** JSON truncation persists with 3+ positions despite optimizations
