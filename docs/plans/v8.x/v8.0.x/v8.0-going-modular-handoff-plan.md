...
### Expected Outcomes
- ✅ Core instructions: 85K → 47K tokens (45% reduction)
- ✅ Simple sessions: 85K → 47K tokens (45% savings)
- ✅ Phase 1 sessions: 85K → 59K tokens (30% savings)
- ✅ Complex sessions: 85K → 77K tokens (9% savings, still better)
...
4. **Enable scalability** - easy to add new optimization-tools/features
...
|---------|--------|---------|----------|
| System Behavior & Product Info | 3,000 | 4% | ❌ Keep |
| Entry Point Routing | 2,500 | 3% | ❌ Keep |
| Phase 1-3 Core Logic | 15,000 | 18% | ❌ Keep |
| **Phase 1 Display Rules** | **12,000** | **14%** | **✅ Extract** |
| **Phase 3 Fit Assessment** | **8,000** | **9%** | **✅ Extract** |
| **Quality Gates & Guardrails** | **10,000** | **12%** | **✅ Extract** |
| **Job History Template System** | **8,000** | **9%** | **✅ Extract** |
| Character Limits & Rules | 5,000 | 6% | ❌ Keep |
| Action Verb Categories | 2,000 | 2% | ❌ Keep |
| Core Principles | 2,000 | 2% | ❌ Keep |
| Formatting Rules | 5,000 | 6% | ❌ Keep |
| Other Sections | 13,500 | 16% | ❌ Keep |
...
├── phase1-display-rules.md (12K tokens)
│   └── Loaded: Phase 1 full analysis only
├── phase3-fit-assessment.md (8K tokens)
│   └── Loaded: JD comparison only
├── quality-gates-guardrails.md (10K tokens)
│   └── Loaded: Debugging/validation only
└── job-history-template-system.md (8K tokens)
    └── Loaded: Job history generation only
```
...
```bash
# Check for /optimization-tools/ or similar directories
ls -la /mnt/project/optimization-tools/ 2>/dev/null
ls -la /mnt/project/workflow-optimization-tools/ 2>/dev/null
...
1. **Read the existing files**
   ```bash
   cat /mnt/project/optimization-tools/resume-analyzer/*.md
...
```
/mnt/project/
├── PROJECT-INSTRUCTIONS.md (consolidated core)
├── phase1-display-rules.md (new module)
├── phase3-fit-assessment.md (new module)
├── quality-gates-guardrails.md (new module)
├── job-history-template-system.md (new module)
├── artifact-creation-instructions.md (already exists)
├── handoff-opus-truncation-fix.md (already exists)
├── start-issue-tracking.md (already exists)
├── create-handoff-backup.md (already exists)
├── job_history_summaries_v8.txt (already exists)
└── workflow-optimization-tools/ (renamed from /optimization-tools/)
    ├── phase-1/
    ├── phase-2/
    ├── phase-3/
    └── phase-4/
...
- Workflow docs: `workflow-optimization-tools/phase-N/specific-file.md`
...
**Issue #27: Rename /optimization-tools/ directory**
...
**Steps:**
1. Check if /optimization-tools/ directory exists
2. If exists, rename to /workflow-optimization-tools/
...
```bash
# Verify rename
ls -la /mnt/project/workflow-optimization-tools/

# Verify no broken references
grep -r "optimization-tools/" /mnt/project/PROJECT-INSTRUCTIONS.md
```
...
**Testing:**
```
Test 1: Phase 1 analysis → Verify display rules load
Test 2: Bullet optimization → Verify display rules NOT loaded
Test 3: Token count → Verify 47K (not 85K) on simple tasks
```
...
**Testing:**
```
Test 1: JD comparison → Verify fit assessment loads
Test 2: Phase 1 analysis → Verify fit assessment NOT loaded
Test 3: Token count → Verify savings on non-JD sessions
```
...
**Testing:**
```
Test 1: Normal operation → Verify guardrails apply without loading module
Test 2: Debug session → Verify module loads on request
Test 3: Token count → Verify 10K savings on normal sessions
```
...
**Testing:**
```
Test 1: Job history generation → Verify template system loads
Test 2: Bullet optimization → Verify template NOT loaded
Test 3: Token count → Verify 8K savings when not needed
```
...
- ✅ Faster Claude responses (less context)
...
- [ ] Check for existing modular files in /optimization-tools/ or similar
- [ ] Understand token usage goals
- [ ] Create backup of current PROJECT-INSTRUCTIONS.md

### During Implementation
- [ ] Follow issue order (27 → 28 → 29 → 30 → 31 → 32)
...
**👉 START HERE:** Issue #27 - Rename /optimization-tools/ folder (30 minutes)
