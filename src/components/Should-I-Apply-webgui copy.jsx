...
2. KEYWORD EVIDENCE PRINCIPLE: Only include keywords that are EVIDENCED in the job history. Do NOT fabricate experience.
3. CUSTOM KEYWORD EVIDENCE (Guardrail #32): If a keyword from the "USE" list lacks direct evidence in the history, incorporate it LIGHTLY/INTEGRATED into existing achievements. Do NOT make it the central theme.
4. CHARACTER LIMITS: Each bullet must be 100-210 characters.
5. VERB DIVERSITY: Use different action verb categories (Built, Lead, Managed, Improved, Collaborate) across bullets.
6. NO FABRICATION: If a skill is missing from the candidate's experience, do NOT pretend they have it.
7. IGNORE LIST: Do NOT use any keyword from the "IGNORE" list under any circumstances.
8. NARRATIVE FIT (Guardrail #33): After generating, verify if the top 3 hard requirements are addressed. If missing, identify them as "Narrative Gaps".

Generate content following this structure. Return ONLY valid JSON:

{
  ...
  "professionalSummary": {
    "text": "3-4 sentence professional summary optimized for this JD. Include 2+ metrics, 2-3 hard skills, 1-2 soft skills. 80-120 words.",
  ...
  "keywordCoverageReport": {
    "successfullyIncorporated": [
      { "keyword": "keyword1", "location": "Position 1, Bullet 2" }
    ],
    "skippedNotEvidenced": [
      { "keyword": "keyword3", "reason": "Not found in candidate's experience" }
    ]
  },
  "optimizationNotes": "Brief notes on what was optimized and why",
  "narrativeVerification": {
    ...
    "score": 0-100
      ...
      if (data.type === 'exceeded_limit') {
        setSummaryError('Rate limit reached. Please wait and try again later.');
        return;
      }

      if (data.error) {
        setSummaryError(`API Error: ${data.error.message}`);
        return;
      }
